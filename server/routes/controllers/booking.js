...
async function createPayment(booking, toUser, paymentToken = null) {
  ...
  if (paymentToken) {
    // It means first time payment or, user changed credit card
    if (user.customer.id) {
      // If user wanna pay with different card as before
  ...
  } else if (user.customer.id) {
    // If user wanna pay with same card as before
    // Store booking user paymentToken and email to charge later.
    const payment = new Payment({
      fromUser: user,
      toUser,
...
exports.createDateBlockBooking = function (req, res) {
  ...
  const booking = new Booking({ startAt, endAt, user });
...
exports.createBooking = function (req, res) {
  ...
  const { startAt, endAt, days, courseTime, totalPrice, rental, paymentToken } =
    req.body;
  const user = res.locals.user;
  ...
  Rental.findById(rental._id)
    ...
    .exec(async function (err, foundRental) {
      ...
      if (paymentToken) {
        const { err, payment } = await createPayment(
          booking,
          foundRental.user,
          paymentToken
        );
        ...
        if (!payment) {
          return res.status(422).send({
            errors: [{ title: "Payment error!", detail: "No payment data!" }],
          });
        }
